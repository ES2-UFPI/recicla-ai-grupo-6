...
class SolicitarColetaView(generics.CreateAPIView):
    ...
    def perform_create(self, serializer):
        try:
            ...
            raise serializers.ValidationError({"detail": "Perfil de Produtor não encontrado."})
        except AttributeError:
            raise serializers.ValidationError({"detail": "Informação de autenticação não encontrada."})
        except Exception as e:
            raise serializers.ValidationError({"detail": f"Erro inesperado: {e}"})
